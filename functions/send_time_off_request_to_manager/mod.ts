...
export default SlackFunction(
  ...
  async function ({ inputs, action, body, client }) {
    ...
    // send the information to the datastore
    const uuid = crypto.randomUUID();
    const putResp = await client.apps.datastore.put<
      typeof TimeOffDatastore.definition
    >({
      datastore: TimeOffDatastore.name,
      item: {
        id: uuid,
